...
public final class HidatoIO {
    ...
    public static int[][] readHidatoFromInput1x1(){
    	...
    	while (numForats == -1 || numForats > (tamanyi*tamanyj-1)) {
    		if (numForats > (tamanyi*tamanyj-1)) System.out.println("No pot tenir forats a totes les caselles");
    		numForats = getNumero();
    	}
    	...
    	for (int i = 0; i < numCaselles; ++i) {
        	...
        	else matriuHidato[coords[0]][coords[1]] = coords[2];
    	}
    	return matriuHidato;
    }
	...
	private static int[] getNumeros() {
		...
		for (int i = 0; i < values.length; ++i) {
			if (isNumber(values[i])) nums[i] = Integer.parseInt(values[i]);
			else return null;
		}
		return nums;
	}
